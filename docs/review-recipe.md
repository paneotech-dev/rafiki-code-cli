...
4. Runs `rafikicode run` headless on the chosen tier with the review prompt and the diff. The reviewer can read the checkout but is denied file edits, shell commands, web fetches and reads of `.git` through `OPENCODE_PERMISSION`. The example workflow checks out with `persist-credentials: false`, so the job token is not stored in `.git/config` either; keep that line in your own workflow. The checkout is not trusted (the action does not set `RAFIKICODE_TRUST_WORKSPACE`), so its project plugins, tools, provider packages, local MCP servers, formatters and language servers are not loaded; see [workspace trust](security/workspace-trust.md).
