...
4. Runs `rafikicode run` headless on the chosen tier with the review prompt and the diff. The reviewer can read the checkout but is denied file edits, shell commands and web fetches through `OPENCODE_PERMISSION`. The checkout is not trusted (the action does not set `RAFIKICODE_TRUST_WORKSPACE`), so its project plugins, tools, provider packages, local MCP servers, formatters and language servers are not loaded; see [workspace trust](security/workspace-trust.md).
