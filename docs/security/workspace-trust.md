...
| Substitution | `{env:NAME}` and `{file:path}` anywhere in project config | when the config loads; the value can be sent to any URL the config names (a remote MCP server, an instructions URL, another provider) |
...
| `{env:}` of secret names, `{file:}` outside the project | replaced by an empty value, one warning line per reference | same | full substitution |
...
Your own configuration keeps full behaviour: `~/.rafikicode/` (or `$XDG_CONFIG_HOME/rafikicode/`), the directory named by `OPENCODE_CONFIG_DIR`, and the files named by `OPENCODE_CONFIG` and `OPENCODE_CONFIG_CONTENT`. Other directories under your home, such as `~/.opencode/`, are treated like project directories. When the home directory itself is a git checkout (some CI images set `HOME` to the checkout), `~/.rafikicode/` came with the repository and is treated like project configuration too; use `XDG_CONFIG_HOME` or `OPENCODE_CONFIG_DIR` to point at a real user configuration in that case.
...
- The background dependency install into project `.rafikicode/` and `.opencode/` directories still runs (install scripts off); nothing it installs is imported unless the workspace is trusted.
...
Questions for the owners and the security reviewer: keep `"trusted"` as the default; also remove local MCP servers, formatters and language servers at a terminal for untrusted workspaces; add the interactive prompt and the file hash; whether `rafikicode serve` without a password should refuse to start.
