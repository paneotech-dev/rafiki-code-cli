...
describe("policy helpers", () => {
  ...
  test("token to credential", () => {
    ...
    const credential = DeviceFlow.toCredential(
      { access_token: "sk-x", token_type: "bearer", expires_in: 3600, key_id: "k", key_alias: "a", gateway_url: "https://g/v1", console_url: "https://c", owner: { id: "u" } },
      now,
    )
    // A plain http gateway or Console URL off this machine is replaced by the default (gateway-scheme.test.ts).
    expect(credential).toMatchObject({ version: 1, key: "sk-x", key_id: "k", key_alias: "a", gateway_url: "https://g/v1", console_url: "https://c" })
