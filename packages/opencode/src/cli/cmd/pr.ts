...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text([Brand.name, "import", sessionUrl], { nothrow: true }),
          )
    ...
    const code = yield* Effect.promise(
      () =>
        Process.spawn([Brand.name, ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`${Brand.name} exited with code ${code}`))
  }),
})
